...
function getAllPossibleCoordsRook(pieceCoord) {
    ...
    for (var colIdx = pieceCoord.j; rowIdx >= 0; rowIdx--) {
    ...
    for (var colIdx = pieceCoord.j; rowIdx < 8; rowIdx++) {
        var coord = { i: rowIdx, j: colIdx };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    //right
    colIdx = pieceCoord.j + 1;
    for (var rowIdx = pieceCoord.i; colIdx < 8; colIdx++) {
        var coord = { i: rowIdx, j: colIdx };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    //left
    colIdx = pieceCoord.j - 1;
    for (var rowIdx = pieceCoord.i; colIdx >= 0; colIdx--) {
        var coord = { i: rowIdx, j: colIdx };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    for (var rowIdx = pieceCoord.i - 2; rowIdx <= pieceCoord.i + 2; rowIdx++) {
        if (rowIdx < 0 || rowIdx >= gBoard.length) continue;
        for (var colIdx = pieceCoord.j - 2; colIdx <= pieceCoord.j + 2; colIdx++) {
            if (colIdx < 0 || colIdx >= gBoard.length) continue;
            var iAbsDiff = Math.abs(rowIdx - pieceCoord.i);
            var jAbsDiff = Math.abs(colIdx - pieceCoord.j);
            var coord = { i: rowIdx, j: colIdx }
            if (!isEmptyCell(coord)) continue;
            if(iAbsDiff+jAbsDiff !== 3) continue;
            res.push(coord)
        }
    }
    return res;
...
function getAllPossibleCoordsQueen(pieceCoord) {
    ...
    return resBishop.concat(resRook);
}
